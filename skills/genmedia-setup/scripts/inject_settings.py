...
def main():
    ...
    print(f"Enabling necessary APIs (Vertex AI, TTS, Storage) on project {args.project}...")
    api_res = subprocess.run(["gcloud", "services", "enable", 
                              "aiplatform.googleapis.com", 
                              "texttospeech.googleapis.com", 
                              "storage.googleapis.com", 
                              "--project", args.project])
    if api_res.returncode != 0:
        print("Warning: Failed to enable required APIs. Please ensure you have sufficient permissions.", file=sys.stderr)
