...
# 2026-06-08 v0.2.2: Added outputGdocUrl metadata parameter.
---
...
2. Run the included `scripts/setup_scaffold.sh` script to deterministically create the scaffolding, passing the optional Codelab URL, TF Bug ID, Codelab Version/Commit Hash, and output Google Doc URL if known:

    ```bash
    ./scripts/setup_scaffold.sh <YYYYMMDD-frictionlog-CODELAB_TITLE> [CODELAB_URL] [BUG_ID] [CODELAB_VERSION] [OUTPUT_GDOC_URL]
    ```

    This script automatically creates the `codelab/original/`, `codelab/proposed/`, `friction_log/by-page/`, `workbench/`, and `external-repos/` directories. It also initializes the `.env` file, the `BUGS.md` file, the `external-repos/.gitignore`, a `.version` file, and a Kubernetes-like key-value metadata store `friction_log.yaml` populated from the template found in `references/friction_log.yaml` (including fields like `startedAt`, `codelabVersion`, and `outputGdocUrl`).
...
Once all pages have been completed, create a short `README.md` file in the base directory containing a high-level summary of the entire run, and a consolidated `friction_log/README.md` file. Make sure to update the `outputGdocUrl` in `friction_log.yaml` if the friction log is compiled/published to a Google Doc.
